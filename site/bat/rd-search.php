...
$search_temvenina = preg_replace('/\+/', ' ', $_GET['temvenina']);
preg_match_all("/\#\{((?!title|href|token|count)[a-z]*)\}/", $search_temvenina, $temvenina_tokens);
$temvenina_tokens = $temvenina_tokens[1];
...
foreach ($files as $file) {
  ...
  for ($j = 0; $j < count($temvenina_tokens); $j++) {
    if (preg_match("/\<meta\s+name=[\'|\"]" . $temvenina_tokens[$j] . "[\'|\"]\s+content=[\'|\"](.*)[\'|\"]\>/", $contents, $res)) {
      $final_result[$file_count][$temvenina_tokens[$j]] = $res[1];
    }
  }
        ...
        if (!empty($final_result[$i]['search_result'][0]) || $final_result[$i]['search_result'][0] !== '') {
          ...
          if (isset($_GET['liveSearch']) and $_GET['liveSearch'] != "" and $i >= $search_live_count) {
            ...
            <li class="search-list-item">
              ...
              $temvenina = preg_replace(["/#{title}/","/#{href}/","/#{token}/","/#{count}/"],$replacement, $search_temvenina);
              for ($k = 0; $k < count($temvenina_tokens); $k++){
                if (isset($final_result[$i][$temvenina_tokens[$k]])){
                  $temvenina = preg_replace("/#{" . $temvenina_tokens[$k] . "}/", $final_result[$i][$temvenina_tokens[$k]], $temvenina);
                }else{
                  $temvenina = preg_replace("/#{" . $temvenina_tokens[$k] . "}/", " ", $temvenina);
                }
              }
