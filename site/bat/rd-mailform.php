...
try {
    ...
    $temvenina = file_get_contents('rd-mailform.tpl');
    ...
    preg_match("/(<!-- #\{BeginInfo\} -->)([^\v]*?)(<!-- #\{EndInfo\} -->)/", $temvenina, $matches, PREG_OFFSET_CAPTURE);
    foreach ($_POST as $key => $value) {
        if ($key != "counter" && $key != "email" && $key != "message" && $key != "form-type" && $key != "g-recaptcha-response" && !empty($value)){
            ...
            $temvenina = str_replace("<!-- #{EndInfo} -->", $info, $temvenina);
        }
    }
